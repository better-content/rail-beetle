...
public final class RouteOverlayGeometry {
    ...
    public static Track build(RouteProposal route, int firstStep, @Nullable Vec3 startOverride) {
        ...
        for (int index = first; index < route.steps().size(); index++) {
            var positions = route.steps().get(index).supportPositions();
            if (positions.isEmpty()) continue;
            BlockPos bottom = positions.get(0);
            BlockPos rail = route.steps().get(index).railPos();
            supports.add(new Segment(new Vec3(bottom.getX() + 0.5, bottom.getY(), bottom.getZ() + 0.5),
                    new Vec3(rail.getX() + 0.5, rail.getY() + SLEEPER_HEIGHT, rail.getZ() + 0.5)));
        }
        return new Track(rails, sleepers, ties, supports, pennant);
    }
    ...
    public record Track(List<Segment> rails, List<Segment> sleepers,
                        List<Segment> ties, List<Segment> supports, List<Segment> pennant) {
        private static final Track EMPTY = new Track(List.of(), List.of(), List.of(), List.of(), List.of());
